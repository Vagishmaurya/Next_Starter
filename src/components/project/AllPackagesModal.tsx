...
export function AllPackagesModal() {
  ...
  const fetchPackages = useCallback(async () => {
    try {
      ...
      setAllPackagesError(null);

      const response = await packagesService.fetchAllPackages(currentPackageType);
      setAllPackages(response.data.packages);
    } catch (error) {
      setAllPackagesError(error instanceof Error ? error.message : 'Failed to fetch packages');
    } finally {
      setAllPackagesLoading(false);
    }
  }, [setAllPackages, setAllPackagesLoading, setAllPackagesError, currentPackageType]);

  useEffect(() => {
    if (showPackagesModal) {
      fetchPackages();
    }
  }, [showPackagesModal, currentPackageType, fetchPackages]);
      ...
      >
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          >
            ...
            All Your {currentPackageType.toUpperCase()} Packages
            {allPackages.length > 0 && (
              ...
              >
                ({allPackages.length} {currentPackageType} packages)
              </span>
            )}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="mb-4">
          <label
            htmlFor="all-package-type-select"
          ...
          <Select value={currentPackageType} onValueChange={setCurrentPackageType}>
            <SelectTrigger id="all-package-type-select" className="w-48">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="overflow-y-auto max-h-[calc(80vh-180px)] pr-2">
          {allPackagesLoading ? (
            ...
            >
              <p className="mb-4">{allPackagesError}</p>
              <Button
            ...
            >
              ...
              <p className={theme === 'dark' ? 'text-zinc-500' : 'text-gray-500'}>
                You don't have any {currentPackageType} packages yet. Create and publish{' '}
                {currentPackageType} packages to see them here.
              </p>
            </div>
          ) : (
